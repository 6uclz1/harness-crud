...
テストは数値(カバレッジ)だけでなく**観点**でも可視化する。CI は各テストの
`describe` / `it` の題名一覧を job summary と PR コメントに毎回出力し
(`scripts/render-test-report.ts`)、人間が「何が検証されているか」を読める状態を保つ。
一方、**結合レベルのテスト観点の導出は CI に組み込まない**。観点の導出は創造的な作業で
exit code で成否を判定できない(LLM の性能に依存する)ため、機械化せず
`.github/pull_request_template.md` で PR 作成者(人間/LLM)に導出させ、人間レビューで
質を担保する。未自動化の観点が繰り返し現れたら integration テストへ昇格させる
(E2E 基盤の導入はその蓄積を見て判断する)。
...
| テスト観点の可視化         | `scripts/render-test-report.ts` + `ci.yml`         |
| 結合レベル観点の導出(PR時) | `.github/pull_request_template.md`                 |
